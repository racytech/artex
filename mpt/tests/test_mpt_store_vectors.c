 ...
 *
 * Scenario types:
 *   0 = build:   insert all keys via mpt_store, verify root.
 *   1 = update:  build initial, apply dirty batch, verify root.
 *   2 = multi:   build initial, apply N rounds of dirty batches,
 *                verify root after each round.
 */
...
/* Apply a dirty batch to the store. Returns true on success. */
static bool apply_dirty_batch(mpt_store_t *ms,
    ...
    return mpt_store_commit_batch(ms);
}
...
int main(int argc, char **argv) {
    ...
    for (uint32_t s = 0; s < num_scenarios; s++) {
        ...
        /* Build the trie */
        cleanup();
        mpt_store_t *ms = mpt_store_create(STORE_PATH, initial_count + 4096);
        if (!ms) {
        ...
        if (scenario_type == 0) {
            ...
            apply_dirty_batch(ms, dirty, dirty_count);
            mpt_store_root(ms, got_root);
            assertions++;
