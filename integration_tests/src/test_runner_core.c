...
bool test_runner_init(test_runner_t *runner, const test_runner_config_t *config) {
    ...
    // Initialize StateDB
    runner->state_db = calloc(1, sizeof(state_db_t));
    if (!runner->state_db) {
        return false;
    }
    
    if (!state_db_init(runner->state_db)) {
        free(runner->state_db);
        runner->state_db = NULL;
        return false;
    }
    
    // Initialize EVM
    runner->evm = evm_create(runner->state_db, NULL); // NULL = use default mainnet config
    if (!runner->evm) {
        state_db_destroy(runner->state_db);
        free(runner->state_db);
        runner->state_db = NULL;
        return false;
    }
...
void test_runner_reset(test_runner_t *runner) {
    if (!runner || !runner->state_db) return;
    
    state_db_reset(runner->state_db);
...
bool test_runner_setup_state(state_db_t *state_db,
                             const test_account_t *accounts,
                             size_t count) {
    if (!state_db || !accounts) return false;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!state_db_set_nonce(state_db, &acc->address, nonce)) {
            return false;
        }
        
        // Set balance
        if (!state_db_set_balance(state_db, &acc->address, &acc->balance)) {
            return false;
        }
        
        // Set code if exists
        if (acc->code && acc->code_len > 0) {
            if (!state_db_set_code(state_db, &acc->address, acc->code, acc->code_len)) {
                printf("ERROR: Failed to set code for account\n");
                return false;
            }
        }
        ...
        for (size_t j = 0; j < acc->storage_count; j++) {
            ...
            if (!state_db_set_state(state_db, &acc->address, &entry->key, &entry->value)) {
                return false;
            }
        }
    }
...
bool test_runner_verify_state_root(state_db_t *state_db,
                                   const hash_t *expected_root,
                                   hash_t *actual_root) {
    if (!state_db || !expected_root) return false;
    
    // Compute current state root
    hash_t computed_root;
    state_db_get_state_root(state_db, &computed_root);
