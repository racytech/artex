...
bool test_runner_run_blockchain_test(test_runner_t *runner,
    ...
    if (runner->config.fork_filter_count > 0 && test->network) {
        ...
        for (size_t i = 0; i < runner->config.fork_filter_count; i++) {
            if (strcmp(runner->config.fork_filter[i], test->network) == 0) {
                fork_allowed = true;
                break;
            }
        }
    ...
    state_db_get_state_root(runner->state_db, &actual_genesis_root);
    ...
    state_db_get_state_root(runner->state_db, &final_state_root);
    ...
    if (test->post_state_count > 0) {
        for (size_t i = 0; i < test->post_state_count; i++) {
            ...
            if (!state_db_get_balance(runner->state_db, &expected_acc->address, &actual_balance)) {
