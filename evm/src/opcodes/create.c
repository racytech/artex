                       ...
                       const uint8_t *code, size_t code_size)
{
    ...
    // Store code in state
    evm_state_set_code(evm->state, contract_addr, code, (uint32_t)code_size);
                                   ...
                                   uint64_t sender_nonce)
{
    ...
    if (success) {
        // Changes stay in cache (no explicit commit needed)
        if (is_problem_addr) {
            printf("DEBUG CREATE: Keeping state for 0x7e2ff...\n");
        }
    } else {
