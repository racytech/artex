 ...
 *     01 = leaf       (0x40000000)
 *     00 = NULL       (ref == 0)
 *   Bits 29-0  : slot index (up to ~1B slots × 64B = 64 GB)
 * ======================================================================== */
...
struct nibble_trie {
    int       fd;
    uint8_t  *slot_base;            /* mmap'd slot pool at file offset 8192 */

    nt_ref_t  root;
    uint64_t  size;                 /* number of key-value pairs */
    uint32_t  slot_count;           /* slots allocated (bump allocator) */

    /* committed-state snapshot (for rollback) */
    nt_ref_t  committed_root;
    uint64_t  committed_size;
    uint32_t  committed_slot_count;
