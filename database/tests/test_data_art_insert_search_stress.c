...
#define TEST_DB_PATH "/tmp/test_data_art_stress_insert_search.db"
...
static void cleanup_test_db(void) {
    ...
    if (result != 0) {
        fprintf(stderr, "WARNING: cleanup command failed with code %d\n", result);
    }
    ...
    if (stat(TEST_DB_PATH, &st) == 0) {
        fprintf(stderr, "WARNING: Database path still exists after cleanup!\n");
    }
}
 ...
 */
static void run_continuous_stress_test(int duration_seconds, bool use_buffer_pool, unsigned int base_seed) {
    printf("\n");
    printf("================================================================\n");
    ...
    while (keep_running && time(NULL) < end_time) {
        ...
        page_manager_t *pm = page_manager_create(TEST_DB_PATH, false);
        if (!pm) {
            FAIL_FAST("Failed to create page manager at iteration %lu", iteration);
        }
        
        // Create buffer pool if requested
        buffer_pool_t *bp = NULL;
        if (use_buffer_pool) {
            buffer_pool_config_t config = buffer_pool_default_config();
            config.capacity = 1000;  // Keep 1000 pages in memory
            bp = buffer_pool_create(&config, pm);
            if (!bp) {
                page_manager_destroy(pm);
                FAIL_FAST("Failed to create buffer pool at iteration %lu", iteration);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        if (duration <= 0) {
            fprintf(stderr, "Usage: %s [duration_in_seconds] [use_buffer_pool] [seed]\n", argv[0]);
            fprintf(stderr, "  No argument: Run all individual tests\n");
            fprintf(stderr, "  With duration: Run continuous stress test for N seconds\n");
            fprintf(stderr, "  With use_buffer_pool: 1 to enable buffer pool, 0 to disable (default: 0)\n");
        ...
        run_continuous_stress_test(duration, use_buffer_pool, base_seed);
        return 0;
    }
