...
// ============================================================================
// Mock Cursor (array-backed, for ih_update tests)
// ============================================================================

typedef struct {
    const uint8_t *const *keys;  // sorted 32-byte keys
...
static bool mock_seek(void *ctx, const uint8_t *seek_key, size_t key_len) {
    ...
    (void)key_len;
    // Binary search for first key >= seek_key
...
// ============================================================================
// Phase 9: ih_update — single value update
// ============================================================================

static void test_update_single_value(void) {
    int start = assertions;
    printf("Phase 9: ih_update — single value update\n");

    // Build initial trie with 5 keys
    uint8_t key_data[5][32];
    memset(key_data, 0, sizeof(key_data));
    key_data[0][0] = 0x11;
    key_data[1][0] = 0x22;
    ...
    hash_t root_update = ih_update(ih, dirty_keys, dirty_vals, dirty_vlens, 1, &cursor);

    // Compare against full rebuild with updated values
    ih_state_t *ih2 = ih_create();
    ...
    hash_t root_rebuild = ih_build(ih2, keys, all_vals, all_vlens, 5);
...
// ============================================================================
// Phase 10: ih_update — multiple value updates
// ============================================================================

static void test_update_multiple_values(void) {
    int start = assertions;
    printf("Phase 10: ih_update — multiple value updates\n");
    ...
    ih_state_t *ih = ih_create();
    ih_build(ih, keys, values, value_lens, 10);
    ...
    hash_t root_update = ih_update(ih, dirty_keys, dirty_vals, dirty_vlens, 3, &cursor);

    // Full rebuild
    ih_state_t *ih2 = ih_create();
    uint16_t all_vlens[10];
    for (int i = 0; i < 10; i++) all_vlens[i] = (uint16_t)cursor_vlens[i];
    hash_t root_rebuild = ih_build(ih2, keys, cursor_vals, all_vlens, 10);
...
// ============================================================================
// Phase 11: ih_update — insert new key into existing branch
// ============================================================================

static void test_update_insert(void) {
    int start = assertions;
    printf("Phase 11: ih_update — insert new key\n");
    ...
    key_data[1][0] = 0x33;
    key_data[2][0] = 0x55;
    key_data[3][0] = 0x22;  // new key to insert (sorted between [0] and [1])
    ...
    // Full rebuild with all 4 keys
    ih_state_t *ih2 = ih_create();
    uint16_t vlens4_u16[] = { 3, 3, 3, 3 };
    hash_t root_rebuild = ih_build(ih2, keys4, vals4, vlens4_u16, 4);
...
// ============================================================================
// Phase 12: ih_update — insert creating new branch slot
// ============================================================================

static void test_update_insert_new_slot(void) {
    int start = assertions;
    printf("Phase 12: ih_update — insert into empty branch slot\n");

    // Start with 2 keys sharing nibble prefix 0x3
    uint8_t key_data[3][32];
    memset(key_data, 0, sizeof(key_data));
    key_data[0][0] = 0x3A;  // nibbles [3, A, ...]
    key_data[1][0] = 0x3B;  // nibbles [3, B, ...]
    key_data[2][0] = 0x3C;  // new key: nibbles [3, C, ...] — new slot in branch
    ...
    ih_state_t *ih = ih_create();
    ih_build(ih, keys2, vals2, vlens2, 2);
    ...
    hash_t root_rebuild = ih_build(ih2, keys3, vals3, vlens3_u16, 3);
...
// ============================================================================
// Phase 13: ih_update — insert splitting extension
// ============================================================================

static void test_update_extension_split(void) {
    int start = assertions;
    printf("Phase 13: ih_update — insert splitting extension\n");

    // Start with 2 keys sharing nibble 3: 0x3A..., 0x3B...
    // Branch at depth 1 with extension [3] at depth 0
    uint8_t key_data[3][32];
    memset(key_data, 0, sizeof(key_data));
    key_data[0][0] = 0x3A;
    key_data[1][0] = 0x3B;
    key_data[2][0] = 0x40;  // diverges at nibble 0: 4 vs 3
    ...
    // Insert 0x40 — this splits the extension at nibble 0
    const uint8_t *dirty_keys[] = { key_data[2] };
    const uint8_t *dirty_vals[] = { (const uint8_t *)"cc" };
    size_t dirty_vlens[] = { 2 };
...
// ============================================================================
// Phase 14: ih_update — sequential blocks
// ============================================================================

static void test_update_sequential_blocks(void) {
    int start = assertions;
    printf("Phase 14: ih_update — sequential blocks\n");
    ...
    key_data[3][0] = 0x44;
    key_data[4][0] = 0x55;
    key_data[5][0] = 0x66;  // will be inserted in block 2
    ...
    ih_state_t *ih = ih_create();
    ih_build(ih, keys, values, vlens, 5);
    ...
    // Block 1: update keys[0] and keys[3]
    {
        ...
        ih_cursor_t cursor = make_mock_cursor(&mc);
        hash_t r1 = ih_update(ih, dk, dv, dvl, 2, &cursor);

        // Verify against full rebuild
        ih_state_t *ref = ih_create();
        uint16_t rvl[5] = {2,2,2,2,2};
        hash_t r1_ref = ih_build(ref, keys, cv, rvl, 5);

        ASSERT(hash_equal(&r1, &r1_ref));
        ih_destroy(ref);

        // Update local values for next block
        values[0] = dv[0];
        values[3] = dv[1];
    }
    ...
    // Block 2: insert key[5] = 0x66
    {
        ...
        ih_cursor_t cursor = make_mock_cursor(&mc);
        hash_t r2 = ih_update(ih, dk, dv, dvl, 1, &cursor);

        // Verify against full rebuild
        ih_state_t *ref = ih_create();
        uint16_t rvl[6] = {2,2,2,2,2,2};
        hash_t r2_ref = ih_build(ref, ck, cv, rvl, 6);
...
// ============================================================================
// Phase 15: ih_update — all keys dirty
// ============================================================================

static void test_update_all_dirty(void) {
    int start = assertions;
    printf("Phase 15: ih_update — all keys dirty\n");
    ...
    ih_state_t *ih = ih_create();
    ih_build(ih, keys, orig_vals, orig_vlens, 5);

    // All 5 keys get new values
    ...
    hash_t root_update = ih_update(ih, dirty_keys, dirty_vals, dirty_vlens, 5, &cursor);

    // Full rebuild
    ih_state_t *ih2 = ih_create();
    uint16_t new_vlens[5];
    for (int i = 0; i < 5; i++) new_vlens[i] = (uint16_t)dirty_vlens[i];
    hash_t root_rebuild = ih_build(ih2, keys, dirty_vals, new_vlens, 5);
...
// ============================================================================
// Phase 16: ih_update — keys with shared nibble prefix
// ============================================================================

static void test_update_shared_prefix(void) {
    int start = assertions;
    printf("Phase 16: ih_update — shared prefix, deep branch\n");

    // Keys with shared first byte: 0xAA11, 0xAA22, 0xAA33, 0xBB11
    // Branch at depth 0 (nibble A vs B), branch at depth 2 (nibble 1 vs 2 vs 3)
    uint8_t key_data[4][32];
    memset(key_data, 0, sizeof(key_data));
    key_data[0][0] = 0xAA; key_data[0][1] = 0x11;
    ...
    ih_state_t *ih = ih_create();
    ih_build(ih, keys, values, vlens, 4);

    // Update key[1] (0xAA22) — touches deep branch under 0xAA
    ...
    hash_t root_update = ih_update(ih, dk, dv, dvl, 1, &cursor);

    // Full rebuild
    ih_state_t *ih2 = ih_create();
    uint16_t rvl[4] = {3,3,3,3};
    hash_t root_rebuild = ih_build(ih2, keys, cv, rvl, 4);
