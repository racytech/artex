...
TEST(test_tree_init_destroy) {
    ...
    ASSERT_TRUE(mem_art_init(&tree), "Tree initialization should succeed");
    ASSERT_NULL(tree.root, "Root should be NULL after init");
    ASSERT_EQ(tree.size, 0, "Size should be 0 after init");
    ASSERT_TRUE(mem_art_is_empty(&tree), "Tree should be empty after init");
...
TEST(test_insert_get_single) {
    ...
    ASSERT_TRUE(mem_art_insert(&tree, key, sizeof(key), value, sizeof(value)),
                "Insert should succeed");
    ASSERT_EQ(mem_art_size(&tree), 1, "Size should be 1 after insert");
    ASSERT_FALSE(mem_art_is_empty(&tree), "Tree should not be empty");
    
    size_t value_len = 0;
    const void *result = mem_art_get(&tree, key, sizeof(key), &value_len);
...
TEST(test_insert_update) {
    ...
    // First insert
    ASSERT_TRUE(mem_art_insert(&tree, key, sizeof(key), value1, sizeof(value1)),
                "First insert should succeed");
    ASSERT_EQ(mem_art_size(&tree), 1, "Size should be 1");
    
    // Update with different value
    ASSERT_TRUE(mem_art_insert(&tree, key, sizeof(key), value2, sizeof(value2)),
                "Update should succeed");
    ASSERT_EQ(mem_art_size(&tree), 1, "Size should still be 1 after update");
...
TEST(test_insert_multiple) {
    ...
    ASSERT_EQ(mem_art_size(&tree), 100, "Size should be 100 after 100 inserts");
...
TEST(test_contains) {
    ...
    mem_art_insert(&tree, key1, sizeof(key1), value, sizeof(value));
    mem_art_insert(&tree, key2, sizeof(key2), value, sizeof(value));
    
    ASSERT_TRUE(mem_art_contains(&tree, key1, sizeof(key1)), "key1 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, key2, sizeof(key2)), "key2 should exist");
    ASSERT_FALSE(mem_art_contains(&tree, key3, sizeof(key3)), "key3 should not exist");
...
TEST(test_delete_single) {
    ...
    mem_art_insert(&tree, key, sizeof(key), value, sizeof(value));
    ASSERT_EQ(mem_art_size(&tree), 1, "Size should be 1 after insert");
    
    ASSERT_TRUE(mem_art_delete(&tree, key, sizeof(key)), "Delete should succeed");
    ASSERT_EQ(mem_art_size(&tree), 0, "Size should be 0 after delete");
    ASSERT_TRUE(mem_art_is_empty(&tree), "Tree should be empty after delete");
    ASSERT_FALSE(mem_art_contains(&tree, key, sizeof(key)), "Key should not exist after delete");
...
TEST(test_delete_nonexistent) {
    ...
    ASSERT_FALSE(mem_art_delete(&tree, key2, sizeof(key2)), "Delete non-existent key should return false");
    ASSERT_EQ(mem_art_size(&tree), 1, "Size should still be 1");
    ASSERT_TRUE(mem_art_contains(&tree, key1, sizeof(key1)), "Original key should still exist");
...
TEST(test_delete_multiple) {
    ...
    ASSERT_EQ(mem_art_size(&tree), 50, "Size should be 50");
    ...
    ASSERT_EQ(mem_art_size(&tree), 25, "Size should be 25 after deleting half");
    
    // Verify remaining items exist and deleted items don't
    for (int i = 0; i < 50; i++) {
        uint8_t key[4] = {0, 0, (i >> 8) & 0xFF, i & 0xFF};
        if (i % 2 == 0) {
            ASSERT_FALSE(mem_art_contains(&tree, key, sizeof(key)), "Deleted key should not exist");
        } else {
            ASSERT_TRUE(mem_art_contains(&tree, key, sizeof(key)), "Non-deleted key should exist");
        }
    }
...
TEST(test_node_growth_4_to_16) {
    ...
    ASSERT_EQ(mem_art_size(&tree), 10, "Size should be 10");
    
    // Verify all keys exist
    for (int i = 0; i < 10; i++) {
        uint8_t key[2] = {0xFF, i};
        ASSERT_TRUE(mem_art_contains(&tree, key, sizeof(key)), "All keys should exist");
    }
...
TEST(test_node_growth_16_to_48) {
    ...
    ASSERT_EQ(mem_art_size(&tree), 30, "Size should be 30");
...
TEST(test_node_growth_48_to_256) {
    ...
    ASSERT_EQ(mem_art_size(&tree), 100, "Size should be 100");
...
TEST(test_variable_length_keys) {
    ...
    ASSERT_TRUE(mem_art_insert(&tree, key1, sizeof(key1), value, sizeof(value)), "Insert key1");
    ASSERT_TRUE(mem_art_insert(&tree, key2, sizeof(key2), value, sizeof(value)), "Insert key2");
    ASSERT_TRUE(mem_art_insert(&tree, key3, sizeof(key3), value, sizeof(value)), "Insert key3");
    ASSERT_TRUE(mem_art_insert(&tree, key4, sizeof(key4), value, sizeof(value)), "Insert key4");
    
    ASSERT_EQ(mem_art_size(&tree), 4, "Size should be 4");
    
    ASSERT_TRUE(mem_art_contains(&tree, key1, sizeof(key1)), "key1 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, key2, sizeof(key2)), "key2 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, key3, sizeof(key3)), "key3 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, key4, sizeof(key4)), "key4 should exist");
...
TEST(test_common_prefix_keys) {
    ...
    mem_art_insert(&tree, key1, sizeof(key1), value1, sizeof(value1));
    mem_art_insert(&tree, key2, sizeof(key2), value2, sizeof(value2));
    mem_art_insert(&tree, key3, sizeof(key3), value3, sizeof(value3));
    mem_art_insert(&tree, key4, sizeof(key4), value4, sizeof(value4));
...
TEST(test_empty_tree_operations) {
    ...
    ASSERT_NULL(mem_art_get(&tree, key, sizeof(key), NULL), "Get on empty tree should return NULL");
    ASSERT_FALSE(mem_art_contains(&tree, key, sizeof(key)), "Contains on empty tree should return false");
    ASSERT_FALSE(mem_art_delete(&tree, key, sizeof(key)), "Delete on empty tree should return false");
    ASSERT_EQ(mem_art_size(&tree), 0, "Size should be 0");
    ASSERT_TRUE(mem_art_is_empty(&tree), "Tree should be empty");
...
TEST(test_ethereum_address_keys) {
    ...
    mem_art_insert(&tree, addr1, sizeof(addr1), &balance1, sizeof(balance1));
    mem_art_insert(&tree, addr2, sizeof(addr2), &balance2, sizeof(balance2));
    mem_art_insert(&tree, addr3, sizeof(addr3), &balance3, sizeof(balance3));
    
    ASSERT_EQ(mem_art_size(&tree), 3, "Should have 3 addresses");
    
    size_t len = 0;
    const uint64_t *result = NULL;
    
    result = mem_art_get(&tree, addr1, sizeof(addr1), &len);
    ASSERT_NOT_NULL(result, "addr1 should be found");
    if (result) ASSERT_EQ(*result, balance1, "addr1 balance should match");
    
    result = mem_art_get(&tree, addr2, sizeof(addr2), &len);
    ASSERT_NOT_NULL(result, "addr2 should be found");
    if (result) ASSERT_EQ(*result, balance2, "addr2 balance should match");
    
    result = mem_art_get(&tree, addr3, sizeof(addr3), &len);
...
TEST(test_storage_slot_keys) {
    ...
    mem_art_insert(&tree, slot1, sizeof(slot1), value1, sizeof(value1));
    mem_art_insert(&tree, slot2, sizeof(slot2), value2, sizeof(value2));
    mem_art_insert(&tree, slot3, sizeof(slot3), value3, sizeof(value3));
    
    ASSERT_EQ(mem_art_size(&tree), 3, "Should have 3 storage slots");
    ASSERT_TRUE(mem_art_contains(&tree, slot1, sizeof(slot1)), "slot1 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, slot2, sizeof(slot2)), "slot2 should exist");
    ASSERT_TRUE(mem_art_contains(&tree, slot3, sizeof(slot3)), "slot3 should exist");
...
TEST(test_iterator_empty_tree) {
    ...
    bool has_next = mem_art_iterator_next(iter);
    ASSERT_FALSE(has_next, "Empty tree should have no elements");
    ASSERT_TRUE(mem_art_iterator_done(iter), "Iterator should be done");
...
TEST(test_iterator_single_element) {
    ...
    const uint8_t *iter_key = mem_art_iterator_key(iter, &key_len);
    const void *iter_value = mem_art_iterator_value(iter, &value_len);
    ...
    has_next = mem_art_iterator_next(iter);
...
TEST(test_iterator_multiple_elements) {
    ...
    mem_art_insert(&tree, key1, sizeof(key1), val1, sizeof(val1));
    mem_art_insert(&tree, key2, sizeof(key2), val2, sizeof(val2));
    mem_art_insert(&tree, key3, sizeof(key3), val3, sizeof(val3));
    mem_art_insert(&tree, key4, sizeof(key4), val4, sizeof(val4));
...
TEST(test_iterator_with_common_prefixes) {
    ...
    mem_art_insert(&tree, key1, sizeof(key1), val, sizeof(val));
    mem_art_insert(&tree, key2, sizeof(key2), val, sizeof(val));
    mem_art_insert(&tree, key3, sizeof(key3), val, sizeof(val));
    mem_art_insert(&tree, key4, sizeof(key4), val, sizeof(val));
...
TEST(test_foreach_empty_tree) {
    ...
    mem_art_foreach(&tree, foreach_empty_callback, &callback_count);
...
TEST(test_foreach_multiple_elements) {
    ...
    int callback_count = 0;
    mem_art_foreach(&tree, foreach_count_callback, &callback_count);
...
TEST(test_foreach_early_termination) {
    ...
    int callback_count = 0;
    mem_art_foreach(&tree, foreach_terminate_callback, &callback_count);
