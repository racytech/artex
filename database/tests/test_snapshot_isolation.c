...
static data_art_tree_t *create_test_tree(const char *db_path, const char *wal_path,
                                          page_manager_t **pm_out, buffer_pool_t **bp_out, wal_t **wal_out) {
    ...
    wal_config.segment_size = 1 * 1024 * 1024;
    wal_t *wal = wal_open(wal_path, &wal_config);
    assert(wal != NULL);
    
    data_art_tree_t *tree = data_art_create(pm, bp, wal, 20);  // Use valid key_size for Ethereum
    assert(tree != NULL);
    assert(tree->mvcc_manager != NULL);
    
    if (pm_out) *pm_out = pm;
    if (bp_out) *bp_out = bp;
    if (wal_out) *wal_out = wal;
    
    return tree;
}

// Helper to destroy tree and components
static void destroy_test_tree(data_art_tree_t *tree, page_manager_t *pm, buffer_pool_t *bp, wal_t *wal,
                               const char *db_path, const char *wal_path) {
...
void test_snapshot_isolation_basic(void) {
    ...
    buffer_pool_t *bp;
    wal_t *wal;
    data_art_tree_t *tree = create_test_tree("test_snapshot.db", "test_snapshot_wal", &pm, &bp, &wal);
    ...
    destroy_test_tree(tree, pm, bp, wal, "test_snapshot.db", "test_snapshot_wal");
...
void test_snapshot_isolation_deletes(void) {
    ...
    buffer_pool_t *bp;
    wal_t *wal;
    data_art_tree_t *tree = create_test_tree("test_snapshot2.db", "test_snapshot2_wal", &pm, &bp, &wal);
    ...
    destroy_test_tree(tree, pm, bp, wal, "test_snapshot2.db", "test_snapshot2_wal");
...
void test_multiple_snapshots(void) {
    ...
    buffer_pool_t *bp;
    wal_t *wal;
    data_art_tree_t *tree = create_test_tree("test_snapshot3.db", "test_snapshot3_wal", &pm, &bp, &wal);
    ...
    destroy_test_tree(tree, pm, bp, wal, "test_snapshot3.db", "test_snapshot3_wal");
