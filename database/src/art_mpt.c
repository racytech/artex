...
/* =========================================================================
 * Hash cache — sparse array indexed by ref offset (direct addressing)
 *
 * O(1) lookup with no hashing or probing. Grows on demand but capped
 * at CACHE_MAX_CAP to prevent unbounded RSS from large arenas.
 * ========================================================================= */

#define CACHE_MAX_CAP  (8 * 1024 * 1024)  /* 8M entries × 34B = ~272MB max */
...
static void cache_put(art_mpt_t *am, art_ref_t ref,
    ...
    size_t idx = ref_to_idx(ref);
    if (idx >= CACHE_MAX_CAP) return;  /* refuse to grow beyond cap */

    if (idx >= am->cache_cap) {
        size_t new_cap = am->cache_cap ? am->cache_cap * 2 : 4096;
        while (new_cap <= idx) new_cap *= 2;
        if (new_cap > CACHE_MAX_CAP) new_cap = CACHE_MAX_CAP;
