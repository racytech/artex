...
int64_t data_art_mlock_hot_pages(data_art_tree_t *tree, size_t budget_bytes,
    ...
    if (!ms->base || ms->mapped_size == 0) return 0;

    /* Range-based mlock: pin the first `budget_bytes` of the file.
     *
     * Inner nodes are allocated before leaf nodes (tree grows top-down),
     * so they cluster at low page IDs.  Locking a contiguous prefix of
     * the file is O(1) — no tree traversal needed — and naturally covers
     * most inner nodes.
     *
     * Also set MADV_RANDOM on the entire region to prevent the kernel
     * from doing useless readahead on random ART traversals. */
    size_t lock_size = ms->mapped_size;
    if (budget_bytes > 0 && budget_bytes < lock_size)
        lock_size = budget_bytes;

    /* Page-align */
    lock_size = lock_size & ~(size_t)(PAGE_SIZE - 1);
    if (lock_size == 0) return 0;

    /* Set MADV_RANDOM on the full mapped region — tells kernel not to
     * readahead, which wastes I/O bandwidth on random access patterns. */
    madvise(ms->base, ms->mapped_size, MADV_RANDOM);

    int rc = mlock(ms->base, lock_size);
    if (rc != 0) {
        /* mlock can fail if we exceed RLIMIT_MEMLOCK.  Try smaller. */
        LOG_INFO("mlock_hot_pages: mlock(%zu MB) failed: %s, trying half",
                 lock_size >> 20, strerror(errno));
        lock_size /= 2;
        lock_size = lock_size & ~(size_t)(PAGE_SIZE - 1);
        if (lock_size > 0)
            rc = mlock(ms->base, lock_size);
    }

    size_t pages_locked = (rc == 0) ? lock_size / PAGE_SIZE : 0;
    size_t bytes_locked = (rc == 0) ? lock_size : 0;

    if (result) {
        result->pages_locked = pages_locked;
        result->bytes_locked = bytes_locked;
        result->inner_nodes_visited = 0;
        result->leaves_skipped = 0;
    }

    if (rc == 0) {
        LOG_INFO("mlock_hot_pages: locked %.1f MB (%.1f GB), MADV_RANDOM set",
                 bytes_locked / (1024.0 * 1024.0),
                 bytes_locked / (1024.0 * 1024.0 * 1024.0));
    }
